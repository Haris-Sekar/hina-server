...
const getCompanyDetails = async (req, res) => {
    try {
        ...
        if ((!company || company.length === 0) && req.user.organization_id) {
            const userCompany = await Organization.findByPk(req.user.organization_id);
            if (userCompany) {
                return res.status(200).json(successResponse([userCompany]));
            } else {
